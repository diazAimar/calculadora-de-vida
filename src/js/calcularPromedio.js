...
const chequearInputs = () => {
  ...
  if (clase !== 'Elegí una clase' && raza !== 'Elegí una raza' && nivel > 0 && vida > 0) {
    proceda = true;
  } else {
    crearAlerta('Todos los campos deben estar completados.', 'warning');
    setTimeout(() => {
...
const calcularVida = (clase, raza, nivel, vida) => {
  ...
  let vidaPromedio = (nivel - 1) * informacion[raza].promedio[clase] + vidaInicial;

  vida > vidaPromedio
    ? crearAlerta('Felicitaciones 🎉! Tu personaje esta +' + (vida - vidaPromedio) + '.', 'success')
    : vida == vidaPromedio
    ? crearAlerta('Tu vida esta en el promedio.', 'warning')
    : crearAlerta('Tu vida está por debajo del promedio, ' + (vida - vidaPromedio) + '.', 'error');
  // console.log(`El promedio de vida de un ${clase} ${raza} nivel ${nivel} es de ${vidaPromedio}`);
};
